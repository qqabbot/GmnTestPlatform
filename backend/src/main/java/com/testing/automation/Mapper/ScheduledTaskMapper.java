...
public interface ScheduledTaskMapper {
        ...
        @Select("SELECT t.*, s.name as scenarioName FROM scheduled_task t " +
                        "LEFT JOIN test_scenarios s ON t.scenario_id = s.id " +
                        "ORDER BY t.id DESC")
        ...
        @Insert("INSERT INTO scheduled_task (name, cron_expression, scenario_id, env_key, status, last_run_time, next_run_time, created_at) "
                        +
                        "VALUES (#{name}, #{cronExpression}, #{scenarioId}, #{envKey}, #{status}, #{lastRunTime}, #{nextRunTime}, #{createdAt})")
        @Options(useGeneratedKeys = true, keyProperty = "id")
        int insert(ScheduledTask task);

        @Update("UPDATE scheduled_task SET name = #{name}, cron_expression = #{cronExpression}, " +
                        "scenario_id = #{scenarioId}, env_key = #{envKey}, status = #{status}, " +
