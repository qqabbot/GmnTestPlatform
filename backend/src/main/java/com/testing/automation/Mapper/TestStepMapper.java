...
public interface TestStepMapper {
    ...
    @Insert("INSERT INTO test_step (case_id, step_name, method, url, headers, body, assertion_script, enabled, step_order, created_at, updated_at) "
            +
            "VALUES (#{caseId}, #{stepName}, #{method}, #{url}, #{headers}, #{body}, #{assertionScript}, #{enabled}, #{stepOrder}, #{createdAt}, #{updatedAt})")
    ...
    @Update("UPDATE test_step SET step_name = #{stepName}, method = #{method}, url = #{url}, " +
            ...
            "enabled = #{enabled}, step_order = #{stepOrder}, updated_at = #{updatedAt} WHERE id = #{id}")
